# scraper to pull student results from https://exam.msrit.edu/
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(top, textvariable=self.year_var, width=6).grid(row=0, column=1, padx=(4, 16))
        ...
        ttk.Entry(top, textvariable=self.start_var, width=6).grid(row=0, column=5, padx=(4, 16))

        ttk.Label(top, text="Count (blank = auto)").grid(row=0, column=6, sticky="w")
        self.count_var = tk.StringVar()
        ttk.Entry(top, textvariable=self.count_var, width=6).grid(row=0, column=7, padx=(4, 16))

        btn_frame = ttk.Frame(self, padding=(10, 0, 10, 6))
        btn_frame.pack(fill="x")

        self.fetch_btn = ttk.Button(btn_frame, text="Fetch", command=self._on_fetch)
        self.fetch_btn.pack(side="left")

        self.stop_btn = ttk.Button(btn_frame, text="Stop", command=self._on_stop, state="disabled")
        self.stop_btn.pack(side="left", padx=6)

        self.export_btn = ttk.Button(btn_frame, text="Export HTML", command=self._on_export, state="disabled")
        self.export_btn.pack(side="left", padx=6)

        self.clear_btn = ttk.Button(btn_frame, text="Clear", command=self._on_clear)
        self.clear_btn.pack(side="left", padx=6)

        self.ranklist_btn = ttk.Button(btn_frame, text="Create Rank List", command=self._on_create_ranklist, state="disabled")
        ...
        self.tree = ttk.Treeview(tree_frame,
            columns=("usn", "name", "sem", "cr_reg", "cr_earn", "sgpa", "cgpa", "subjects"),
    ...
    def _validate_inputs(self):
        ...
        if not year.isdigit() or len(year) != 2:
            messagebox.showerror("Invalid input", "Year must be exactly 2 digits.")
            return None
        if branch not in BRANCHES:
            messagebox.showerror("Invalid input", "Pick a valid branch.")
            return None
        if not start_str.isdigit() or int(start_str) < 1:
            messagebox.showerror("Invalid input", "Start must be a positive integer.")
            return None

        count = None
        if count_str:
            if not count_str.isdigit() or int(count_str) < 1:
                messagebox.showerror("Invalid input", "Count must be a positive integer or blank.")
                return None
            count = int(count_str)
    ...
    def _on_fetch(self):
        ...
        # rank list only makes sense when pulling the whole branch from roll no. 1
        self._full_branch_fetch = (start == 1 and count is None)
        ...
        self._thread = threading.Thread(target=self._fetch_loop, args=params, daemon=True)
        self._thread.start()
    ...
    def _insert_row(self, d):
        subj_str = " | ".join(
            f"{s[0]}: {s[-1]}" if len(s) >= 2 else ", ".join(s)
            for s in d["subjects"]
        )
        values = (d["usn"], d["name"], d["sem"], d["credits_registered"],
                  d["credits_earned"], d["sgpa"], d["cgpa"], subj_str)
    ...
    def _fetch_done(self):
        ...
        if self._results and self._full_branch_fetch:
    ...
    def _on_export(self):
        ...
        for d in self._results:
            subj_rows = "".join(
                "<tr>" + "".join(f"<td>{html.escape(c)}</td>" for c in s) + "</tr>"
                for s in d["subjects"]
            )
            subj_table = (
        ...
        self.status_var.set(f"Exported {len(self._results)} results to {os.path.basename(path)}")
    ...
    def has_backlogs(self, d):
        try:
            return int(d["credits_registered"]) != int(d["credits_earned"])
        except (ValueError, KeyError):
            return False

    def build_ranklist_html(self):
        branch = self.branch_var.get().strip().upper()
        year = self.year_var.get().strip()
        sem = self._results[0]["sem"] if self._results else "?"

        sorted_students = sorted(self._results, key=self.get_cgpa, reverse=True)
        total = len(sorted_students)

        # ties get the same rank
        ranks = []
        for i, d in enumerate(sorted_students):
            if i == 0:
                ranks.append(1)
            else:
                if self.get_cgpa(d) == self.get_cgpa(sorted_students[i - 1]):
                    ranks.append(ranks[-1])
                else:
                    ranks.append(i + 1)

        rows = []
        for rank, d in zip(ranks, sorted_students):
            cgpa = self.get_cgpa(d)
            badge = self.badge_class(cgpa)
            rc = self.row_class(rank)
            warn = " ⚠" if self.has_backlogs(d) else ""

            rows.append(
                f'<tr{rc}>'
                f'<td class="rank">{rank}</td>'
                f'<td class="usn">{html.escape(d["usn"])}</td>'
                f'<td>{html.escape(d["name"])}{html.escape(warn)}</td>'
                f'<td>{html.escape(d["credits_registered"])}</td>'
                f'<td>{html.escape(d["credits_earned"])}</td>'
                f'<td>{html.escape(d["sgpa"])}</td>'
                f'<td><span class="badge {badge}">{html.escape(d["cgpa"])}</span></td>'
                f'</tr>'
            )
  ...
  <div class="legend">
    ...
    def _on_create_ranklist(self):
        ...
        path = filedialog.asksaveasfilename(
            defaultextension=".html",
            filetypes=[("HTML files", "*.html")],
            initialfile=f"MSRIT_{branch}_Sem{sem}_RankList.html",
            title="Save Rank List",
        )
        if not path:
            return

        with open(path, "w", encoding="utf-8") as f:
            f.write(self.build_ranklist_html())

        self.status_var.set(f"Rank list saved — {len(self._results)} students → {os.path.basename(path)}")
